...
var DEFAULT_CARRIER_MAX_FEE = microAlgo(257000n);
...
function registeredAccessCarrierApps(algod) {
  return algod[ACCESS_CARRIER_REGISTRY] ?? /* @__PURE__ */ new Map();
}
...
function installEmptyAccessBoxDecoderCompatibility() {
  var _a;
  const appCallCodec = (_a = TransactionParamsMeta.fields.find((field) => field.name === "appCall")) == null ? void 0 : _a.codec;
  ...
  prototype.encodeResourceReferences = function patchedEncodeResourceReferences(appId, references, format) {
    var _a2;
    const encoded = ((_a2 = originalEncode.call(this, appId, references, format)) == null ? void 0 : _a2.map((reference) => {
...
function isAutomaticCarrierTransaction(transaction) {
  ...
  return Boolean(
    transaction.type === TransactionType.AppCall && note && new TextDecoder().decode(note).startsWith("akta-access-carrier-")
  );
}
function installSingleSendResultCompatibility() {
  const prototype = AlgorandClientTransactionSender.prototype;
...
function registerAccessListResourceCarrier(appId, appSpec, algod) {
  ...
  const methods = (appSpec == null ? void 0 : appSpec.methods) ?? [];
...
function hasLegacyIndexedReferences(transaction) {
  var _a, _b, _c;
  return transaction.type === TransactionType.AppCall && transaction.appCall !== void 0 && ((((_a = transaction.appCall.accountReferences) == null ? void 0 : _a.length) ?? 0) > 0 || (((_b = transaction.appCall.assetReferences) == null ? void 0 : _b.length) ?? 0) > 0 || (((_c = transaction.appCall.appReferences) == null ? void 0 : _c.length) ?? 0) > 0);
}
function legacyReferenceCount(transaction) {
  var _a, _b, _c, _d;
  const appCall = transaction.appCall;
  return (((_a = appCall.accountReferences) == null ? void 0 : _a.length) ?? 0) + (((_b = appCall.assetReferences) == null ? void 0 : _b.length) ?? 0) + (((_c = appCall.appReferences) == null ? void 0 : _c.length) ?? 0) + (((_d = appCall.boxReferences) == null ? void 0 : _d.length) ?? 0);
}
function provideLiteralZeroAddress(transactions) {
  ...
  const appTransactions = transactions.filter(
    (transaction) => transaction.type === TransactionType.AppCall && transaction.appCall !== void 0
  );
  ...
  const candidate = appTransactions.filter((transaction) => {
    var _a2, _b2;
    const appCall = transaction.appCall;
    return (((_a2 = appCall.accessReferences) == null ? void 0 : _a2.length) ?? 0) === 0 && (((_b2 = appCall.accountReferences) == null ? void 0 : _b2.length) ?? 0) < 4 && legacyReferenceCount(transaction) < 8;
  }).sort((first, second) => {
...
function transactionResources(resources, createdAppIds = /* @__PURE__ */ new Set()) {
  ...
  return [
    ...(resources.accounts ?? []).filter((address) => !isZeroAddress(address)).filter((address) => !createdAppAddresses.has(address.toString())).map((address) => ({ address })),
    ...(resources.assets ?? []).map((assetId) => ({ assetId })),
    ...(resources.apps ?? []).filter((appId) => !createdAppIds.has(appId)).map((appId) => ({ appId })),
...
function hasUnnamedResources(resources) {
  ...
  return Boolean(
    resources && ((((_a = resources.accounts) == null ? void 0 : _a.length) ?? 0) > 0 || (((_b = resources.assets) == null ? void 0 : _b.length) ?? 0) > 0 || (((_c = resources.apps) == null ? void 0 : _c.length) ?? 0) > 0 || (((_d = resources.assetHoldings) == null ? void 0 : _d.length) ?? 0) > 0 || (((_e = resources.appLocals) == null ? void 0 : _e.length) ?? 0) > 0 || (((_f = resources.boxes) == null ? void 0 : _f.length) ?? 0) > 0 || (resources.extraBoxRefs ?? 0) > 0)
  );
}
function mergeUnnamedResources(first, second) {
  if (!hasUnnamedResources(first) && !hasUnnamedResources(second)) return void 0;
  const merged = {
    accounts: [...(first == null ? void 0 : first.accounts) ?? [], ...(second == null ? void 0 : second.accounts) ?? []],
    assets: [...(first == null ? void 0 : first.assets) ?? [], ...(second == null ? void 0 : second.assets) ?? []],
    apps: [...(first == null ? void 0 : first.apps) ?? [], ...(second == null ? void 0 : second.apps) ?? []],
    assetHoldings: [...(first == null ? void 0 : first.assetHoldings) ?? [], ...(second == null ? void 0 : second.assetHoldings) ?? []],
    appLocals: [...(first == null ? void 0 : first.appLocals) ?? [], ...(second == null ? void 0 : second.appLocals) ?? []],
    boxes: [...(first == null ? void 0 : first.boxes) ?? [], ...(second == null ? void 0 : second.boxes) ?? []],
    extraBoxRefs: ((first == null ? void 0 : first.extraBoxRefs) ?? 0) + ((second == null ? void 0 : second.extraBoxRefs) ?? 0)
  };
  return merged;
}
function mergeGroupAnalysis(original, repairs, transactionCount) {
  ...
  return {
    transactions: Array.from({ length: transactionCount }, (_, index) => {
      var _a, _b;
      return {
        ...(original == null ? void 0 : original.transactions[index]) ?? {},
    ...
    createdAppIds: [
      .../* @__PURE__ */ new Set([...(original == null ? void 0 : original.createdAppIds) ?? [], ...(repairs == null ? void 0 : repairs.createdAppIds) ?? []])
    ]
  };
}
function filterNewRepairResources(composer, scope, resources) {
  ...
  const add = (field, key, value) => {
    ...
    keys.add(scopedKey);
    const values = filtered[field] ?? [];
    values.push(value);
    filtered[field] = values;
  };
  for (const value of (resources == null ? void 0 : resources.accounts) ?? []) add("accounts", `d:${value.toString()}`, value);
  for (const value of (resources == null ? void 0 : resources.assets) ?? []) add("assets", `s:${value}`, value);
  for (const value of (resources == null ? void 0 : resources.apps) ?? []) add("apps", `p:${value}`, value);
  for (const value of (resources == null ? void 0 : resources.assetHoldings) ?? []) {
    add("assetHoldings", `h:${value.address.toString()}:${value.assetId}`, value);
  }
  for (const value of (resources == null ? void 0 : resources.appLocals) ?? []) {
    add("appLocals", `l:${value.address.toString()}:${value.appId}`, value);
  }
  for (const value of (resources == null ? void 0 : resources.boxes) ?? []) {
    add("boxes", `b:${value.appId}:${bytesKey(value.name)}`, value);
  }
  if (((resources == null ? void 0 : resources.extraBoxRefs) ?? 0) > 0) filtered.extraBoxRefs = resources.extraBoxRefs;
  ...
  for (const transaction of composer.txns) {
    ...
    if (transaction.type === "asyncTxn" && maybePromise instanceof Promise) {
      transaction.data.txn = maybePromise.then((resolved) => {
        const clone = new Transaction({ ...resolved });
        clone.group = void 0;
        return clone;
      });
    } else {
...
function collectCreatedAppIds(group) {
  ...
  for (const transaction of (group == null ? void 0 : group.txnResults) ?? []) visit(transaction.txnResult);
  return [...result];
}
function populateAccessListResources(transactions, groupAnalysis) {
  var _a, _b, _c, _d;
  const appTransactions = transactions.map((transaction, groupIndex) => ({ transaction, groupIndex })).filter(({ transaction }) => transaction.type === TransactionType.AppCall && transaction.appCall !== void 0);
  if (appTransactions.length === 0) return;
  const createdAppIds = new Set((groupAnalysis == null ? void 0 : groupAnalysis.createdAppIds) ?? []);
  ...
  const extraBoxReferences = ((_c = groupAnalysis == null ? void 0 : groupAnalysis.unnamedResourcesAccessed) == null ? void 0 : _c.extraBoxRefs) ?? 0;
  ...
  const requiredCarrierCount = Math.max(requiredResourceCarriers, (groupAnalysis == null ? void 0 : groupAnalysis.requiredOpcodeCarriers) ?? 0);
...
function installAccessListResourcePopulator() {
  ...
  const prototype = TransactionComposer.prototype;
    ...
    prototype.build = async function patchedBuild() {
      ...
      while (true) {
        try {
          ...
          if (!validatedComposers.has(this) && this.composerConfig.populateAppCallResources && result.transactions.some(({ txn }) => txn.type === TransactionType.AppCall)) {
            const unsignedGroup = {
              txnGroups: [
                {
            ...
            if (group == null ? void 0 : group.failureMessage) {
              ...
              throw new Error(
                `Final Access-list validation failed in transaction ${((_a = group.failedAt) == null ? void 0 : _a.join(", ")) ?? "unknown"}: ` + group.failureMessage
              );
            }
          ...
          const anchorIndex = builtTransactions.findIndex(
            (transaction) => transaction.type === TransactionType.AppCall && transaction.appCall !== void 0 && transaction.appCall.appId > 0n && transaction.appCall.onComplete !== OnApplicationComplete.DeleteApplication && registeredApps.has(transaction.appCall.appId.toString())
          );
          const deletingRegisteredApps = new Set(
            builtTransactions.filter(
              (transaction) => {
                var _a2;
                return transaction.type === TransactionType.AppCall && ((_a2 = transaction.appCall) == null ? void 0 : _a2.onComplete) === OnApplicationComplete.DeleteApplication && registeredApps.has(transaction.appCall.appId.toString());
              }
          ...
          if (targetAppId === void 0) {
            const appIds = builtTransactions.filter((transaction) => {
              var _a2;
              return transaction.type === TransactionType.AppCall && ((_a2 = transaction.appCall) == null ? void 0 : _a2.appId);
          ...
          const senderContext = ((_b = this.txns[senderContextIndex]) == null ? void 0 : _b.data) ?? {};
  ...
  prototype.populateTransactionAndGroupResources = function patchedPopulate(transactions, groupAnalysis) {
    ...
    const assumedFees = transactions.map(
      (transaction, groupIndex) => {
        var _a2;
        return algoAmountMicroAlgos((_a2 = this.txns[groupIndex]) == null ? void 0 : _a2.data.maxFee) ?? transaction.fee ?? 0n;
      }
    );
    const feesBeforeAccess = feeParams ? transactions.map(
      (transaction) => byteFeeRequirement(transaction, feeParams, transaction.fee ?? 0n, false)
    ) : [];
    ...
    const coversInnerFees = ((_a = this.composerConfig) == null ? void 0 : _a.coverAppCallInnerTransactionFees) ?? false;
    const resourcesStripped = effectiveAnalysis ? {
      ...effectiveAnalysis,
      transactions: effectiveAnalysis.transactions.map((analysis, groupIndex) => {
        const feeAdjustment = feeParams ? coversInnerFees ? feesAfterAccess[groupIndex] - feesBeforeAccess[groupIndex] : hasAccessFeeGrowth ? feesAfterAccess[groupIndex] - (transactions[groupIndex].fee ?? 0n) : 0n : 0n;
        const feeAdjusted = {
          ...analysis,
...
var BaseSDK = class {
  ...
  getSendParams({ sender, signer } = {}) {
    return {
      ...
      ...signer !== void 0 && { signer: normalizeSigner(signer) }
    };
  }
...
//# sourceMappingURL=chunk-S34CQKA6.mjs.map