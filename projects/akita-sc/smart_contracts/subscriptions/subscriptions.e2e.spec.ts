...
describe('Subscriptions Contract Tests', () => {
  ...
  describe('Service Management', () => {
    ...
    test('activate a draft service', async () => {
      ...
      const updatedService = await subscriptions.getService({ sender, address: sender, id: Number(serviceId) })
    ...
    test('get service that does not exist fails', async () => {
      ...
      let error = 'no error thrown'
      try {
        await subscriptions.getService({ sender, address: sender, id: 9999 })
      } catch (e: any) {
        error = e.message
      }
  ...
  describe('Subscription Lifecycle', () => {
    test('subscribe to a donation (no service)', async () => {
      ...
      const subInfo = await subscriptions.getSubscription({ sender, address: sender, id: subscriptionId })
    ...
    test('deposit funds into subscription', async () => {
      ...
      const subAfter = await subscriptions.getSubscription({ sender, address: sender, id: subscriptionId })
    ...
    test('unsubscribe and get refund', async () => {
      ...
      let error = 'no error thrown'
      try {
        await subscriptions.getSubscription({ sender, address: sender, id: subscriptionId })
      } catch (e: any) {
        error = e.message
      }
    ...
    test('check if first subscription', async () => {
      ...
      const isFirst = await subscriptions.isFirstSubscription({ sender: newAcc.addr.toString(), address: newAcc.addr.toString() })
    ...
    test('verify recipient receives payment on subscribe', async () => {
      ...
      const recipientInfoBefore = await algorand.client.algod.accountInformation(recipientAcc.addr.toString())
      ...
      const recipientInfoAfter = await algorand.client.algod.accountInformation(recipientAcc.addr.toString())
  ...
  describe('Payment Triggering', () => {
    ...
    test('streak resets when payment window is missed', async () => {
      ...
      const subAfterFirst = await subscriptions.getSubscription({ sender, address: sender, id: subscriptionId })
      ...
      const subAfterMiss = await subscriptions.getSubscription({ sender, address: sender, id: subscriptionId })
    ...
    test('multiple trigger payments reduce escrowed balance', async () => {
      ...
      const subInitial = await subscriptions.getSubscription({ sender, address: sender, id: subscriptionId })
      ...
      const subAfterSecond = await subscriptions.getSubscription({ sender, address: sender, id: subscriptionId })
  ...
  describe('Balance Verification & Fee Distribution', () => {
    ...
    const getEscrowAddress = async (algorand: any): Promise<string> => {
      // Get the escrow tuple from subscriptions contract global state
      // Returns { name: string, app: bigint } where app is the escrow app ID
      const escrow = await subscriptions.client.state.global.akitaDaoEscrow()
      if (!escrow || !escrow.app) throw new Error('Escrow app ID not found')
      // Convert app ID to application address
      const { getApplicationAddress } = await import('algosdk')
      return getApplicationAddress(escrow.app).toString()
    }
    ...
    test('verify fee distribution on subscribe', async () => {
      ...
      const recipientInfoBefore = await algorand.client.algod.accountInformation(recipientAcc.addr.toString())
      const recipientBalanceBefore = BigInt(recipientInfoBefore.amount)
      const escrowInfoBefore = await algorand.client.algod.accountInformation(escrowAddress)
      ...
      const recipientInfoAfter = await algorand.client.algod.accountInformation(recipientAcc.addr.toString())
      const recipientBalanceAfter = BigInt(recipientInfoAfter.amount)
      const escrowInfoAfter = await algorand.client.algod.accountInformation(escrowAddress)
    ...
    test('verify fee distribution on trigger payment', async () => {
      ...
      const escrowInfoBefore = await algorand.client.algod.accountInformation(escrowAddress)
      const escrowBalanceBefore = BigInt(escrowInfoBefore.amount)
      const triggerSenderInfoBefore = await algorand.client.algod.accountInformation(sender)
      ...
      const escrowInfoAfter = await algorand.client.algod.accountInformation(escrowAddress)
      const escrowBalanceAfter = BigInt(escrowInfoAfter.amount)
      const triggerSenderInfoAfter = await algorand.client.algod.accountInformation(sender)
    ...
    test('unsubscribe returns full escrowed balance', async () => {
      ...
      const senderInfoBefore = await algorand.client.algod.accountInformation(sender)
      ...
      const senderInfoAfter = await algorand.client.algod.accountInformation(sender)
  ...
  describe('Withdrawal Edge Cases and Security', () => {
    ...
    test('verify contract balance decreases on withdrawal', async () => {
      ...
      const contractInfoBefore = await algorand.client.algod.accountInformation(contractAddress)
      ...
      const contractInfoAfter = await algorand.client.algod.accountInformation(contractAddress)
  ...
  describe('ASA Subscriptions', () => {
    ...
    beforeAll(async () => {
      ...
      testAssetId = BigInt(assetCreateResult.confirmation.assetId!)
      assetDecimals = 6
    ...
    test('withdraw ASA from subscription', async () => {
      ...
      const senderInfoBefore = await algod.accountAssetInformation(sender, Number(testAssetId))
      ...
      const senderInfoAfter = await algod.accountAssetInformation(sender, Number(testAssetId))
  ...
  describe('Payout Address', () => {
    test('service stores custom payout address', async () => {
      const { algorand, context: { testAccount } } = localnet
      const sender = testAccount.toString()
      const signer = testAccount.signer
      ...
      // Create a separate payout account
      const payoutAcc = algorand.account.random()
      await algorand.account.ensureFunded(payoutAcc.addr, dispenser, (10).algos())
      await algorand.account.ensureFunded(sender, dispenser, (200).algos())
      ...
      const serviceId = await subscriptions.newService({
        ...
        passes: 0n,
        gateId: 0n,
        payoutAddress: payoutAcc.addr.toString(),
        title: 'Payout Test Service',
        description: 'Service with custom payout address',
        bannerImage: EMPTY_CID,
        highlightMessage: 0,
        highlightColor: '#FF5733',
      })
      ...
      const service = await subscriptions.getService({ sender, address: sender, id: Number(serviceId) })
      expect(service.payoutAddress).toBe(payoutAcc.addr.toString())
    })

    test('initial subscription payment goes to payout address', async () => {
      const { algorand, context: { testAccount } } = localnet
      const sender = testAccount.toString()
      const signer = testAccount.signer
      ...
      // Create merchant and a separate payout account
      const merchantAcc = algorand.account.random()
      const payoutAcc = algorand.account.random()
      await algorand.account.ensureFunded(merchantAcc.addr, dispenser, (200).algos())
      await algorand.account.ensureFunded(payoutAcc.addr, dispenser, (10).algos())
      ...
      // Merchant creates service with custom payout address
      const serviceId = await subscriptions.newService({
        sender: merchantAcc.addr.toString(),
        signer: merchantAcc.signer,
        interval: BigInt(MIN_INTERVAL),
        ...
        description: 'Service with payout address',
        bannerImage: EMPTY_CID,
        highlightMessage: 0,
        highlightColor: '#000000',
      })

      // Record balances before subscription
      const payoutBalanceBefore = (await algorand.account.getInformation(payoutAcc.addr)).balance
      const merchantBalanceBefore = (await algorand.account.getInformation(merchantAcc.addr)).balance
      ...
      // Check that payout account received funds, not the merchant
      const payoutBalanceAfter = (await algorand.account.getInformation(payoutAcc.addr)).balance
      const merchantBalanceAfter = (await algorand.account.getInformation(merchantAcc.addr)).balance

      expect(payoutBalanceAfter.microAlgo).toBeGreaterThan(payoutBalanceBefore.microAlgo)
      expect(merchantBalanceAfter.microAlgo).toBe(merchantBalanceBefore.microAlgo)
    })

    test('triggered payment goes to payout address', async () => {
      const { algorand, context: { testAccount } } = localnet
      const sender = testAccount.toString()
      const signer = testAccount.signer
      ...
      // Create merchant and separate payout account
      const merchantAcc = algorand.account.random()
      const payoutAcc = algorand.account.random()
      await algorand.account.ensureFunded(merchantAcc.addr, dispenser, (200).algos())
      await algorand.account.ensureFunded(payoutAcc.addr, dispenser, (10).algos())
      ...
      const serviceId = await subscriptions.newService({
        ...
        gateId: 0n,
        payoutAddress: payoutAcc.addr.toString(),
        title: 'Trigger Payout Service',
        description: 'Service with payout address for trigger test',
        bannerImage: EMPTY_CID,
        highlightMessage: 0,
        highlightColor: '#000000',
      })

      // User subscribes with initial deposit for future payments
      const subscriptionId = await subscriptions.subscribe({
        sender,
        signer,
        asset: 0n,
      ...
      // Record balances before trigger
      const payoutBalanceBefore = (await algorand.account.getInformation(payoutAcc.addr)).balance
      const merchantBalanceBefore = (await algorand.account.getInformation(merchantAcc.addr)).balance
      ...
      // Verify payout account received funds, not the merchant
      const payoutBalanceAfter = (await algorand.account.getInformation(payoutAcc.addr)).balance
      const merchantBalanceAfter = (await algorand.account.getInformation(merchantAcc.addr)).balance
    ...
    test('zero payout address defaults payment to service creator', async () => {
      const { algorand, context: { testAccount } } = localnet
      const sender = testAccount.toString()
      const signer = testAccount.signer
      ...
      // Create merchant with no custom payout address (zero address)
      const merchantAcc = algorand.account.random()
      await algorand.account.ensureFunded(merchantAcc.addr, dispenser, (200).algos())
      await algorand.account.ensureFunded(sender, dispenser, (200).algos())
      ...
      const serviceId = await subscriptions.newService({
        ...
        gateId: 0n,
        payoutAddress: 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAY5HFKQ',
        title: 'Default Payout Service',
        description: 'Service with zero payout address',
        bannerImage: EMPTY_CID,
        highlightMessage: 0,
        highlightColor: '#000000',
      })

      // Record merchant balance before subscription
      const merchantBalanceBefore = (await algorand.account.getInformation(merchantAcc.addr)).balance
      ...
      // Merchant should receive payment since payout address is zero
      const merchantBalanceAfter = (await algorand.account.getInformation(merchantAcc.addr)).balance
      expect(merchantBalanceAfter.microAlgo).toBeGreaterThan(merchantBalanceBefore.microAlgo)
    })

    test('getSubscriptionWithDetails returns payout address', async () => {
      const { algorand, context: { testAccount } } = localnet
      const sender = testAccount.toString()
      const signer = testAccount.signer
      ...
      const serviceId = await subscriptions.newService({
        ...
        gateId: 0n,
        payoutAddress: payoutAcc.addr.toString(),
        title: 'Details Payout Service',
        description: 'Service for details test',
        bannerImage: EMPTY_CID,
        highlightMessage: 0,
        highlightColor: '#000000',
      })
      ...
      const details = await subscriptions.getSubscriptionWithDetails({
        sender,
        address: sender,
        id: subscriptionId,
      })

      expect(details.payoutAddress).toBe(payoutAcc.addr.toString())
    })
  })
})
