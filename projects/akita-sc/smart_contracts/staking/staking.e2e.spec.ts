...
const VALID_LOCK_DURATION = ONE_YEAR - ONE_DAY

/**
 * Get the current Algorand block timestamp.
 * This is used instead of Date.now() because localnet block timestamps can drift
 * forward and the staking contract validates expirations against Global.latestTimestamp.
 */
...
const getAssetBalance = async (
  ...
  for (const asset of info.assets ?? []) {
    const record = asset as { ['asset-id']?: bigint | number; assetId?: bigint | number; amount?: bigint | number }
    const holdingAssetId = record['asset-id'] ?? record.assetId
    if (holdingAssetId !== undefined && BigInt(holdingAssetId) === assetId) {
...
describe('Staking Contract', () => {
  ...
  describe('ALGO Staking', () => {
    ...
    describe('Hard Staking (Escrow)', () => {
      test('should create a new hard stake with escrowed ALGO', async () => {
        ...
        const expiration = BigInt(Number(await getBlockTimestamp(algorand)) + VALID_LOCK_DURATION)
      ...
      test('should add to existing hard stake', async () => {
        ...
        const newExpiration = BigInt(Number(await getBlockTimestamp(algorand)) + ONE_DAY * 2)
      ...
      test('should fail lock stake with expiration more than 1 year', async () => {
        ...
        const expiration = BigInt(Number(await getBlockTimestamp(algorand)) + ONE_YEAR + ONE_DAY)
  ...
  describe('Withdraw', () => {
    test('should withdraw hard staked ALGO', async () => {
      const { algorand } = fixture.context
      const stakeAmount = 3_000_000n
      const expiration = BigInt(Number(await getBlockTimestamp(algorand)) + 60) // 1 minute
  ...
  describe('Update Settings', () => {
    test('should allow asset creator to update settings', async () => {
      ...
      const expiration = BigInt(Number(await getBlockTimestamp(algorand)) + Number(maxLockupDuration) - 1000)
