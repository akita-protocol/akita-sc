...
async function getAccountBalances(
  ...
  const info = await algod.accountInformation(address);
 ...
 *
 * `expectedCost` rides along on the single simulate that `prepareGroup` runs
 * during `composer.build()`: a Proxy over the composer's algod client captures
 * the simulate response, and `computeExpectedCost` (simulate/cost.ts) walks
 * top-level + inner txns to aggregate per-asset outflows from the wallet's
 * app account. Zero extra round-trips — the UX flow is:
 *   page load → build.usePlugin (1 simulate, populates `expectedCost`)
 *   user clicks → send() signs & submits the prepared group
 *
 * These tests assert prediction equals the real on-chain delta, so any drift
 * between the simulate-response walker and actual execution surfaces here.
 */
describe('WalletSDK.build.usePlugin cost accuracy', () => {
  ...
  beforeAll(async () => {
    ...
    // Create a test asset owned by sender. Use the utils10 composer path —
    // `algosdk.makeAssetCreateTxnWithSuggestedParamsFromObject` produces a
    // txn that utils10's signer rejects at validation time ("Total is
    // immutable and cannot be changed", etc.) because algosdk's ASA-create
    // output shape no longer matches what utils10's validator expects.
    const { assetId: createdAssetId } = await algorand.send.assetCreate({
      sender,
      signer,
  ...
  describe('asset close-out cost accuracy', () => {
    test('expectedCost.subtotals accounts for full balance on close-out', async () => {
      // Create a new asset for this test to avoid interference. Use utils10's
      // composer — see matching note in beforeAll about algosdk interop.
      const { assetId: closeTestAssetId } = await fixture.algorand.send.assetCreate({
        sender,
        signer,
        total: 100_000_000n,
        decimals: 6,
    ...
    test('ALGO close-out fails for external controlled account (would undo rekey)', async () => {
      ...
      // Rekey external account to the factory BEFORE wallet creation
      // The factory will rekey it to the new wallet during newAccount()
      await fixture.algorand.send.payment({
      ...
      // Factory handles rekeying the external account to the new wallet
